...
echo '<table>
  <tr>
	...
	while($Arrayline = mysqli_fetch_assoc($result)) {
		...
		echo '<tr>';
		 ...
		 echo '<td>';
		  echo '<a href="commision.php?procedure=comms_deleteme&cuid='.$Arrayline['uid'].'" title="This will delete me"><i class="fa fa-trash fa fa-2x" aria-hidden="true"></i></a>';
		  echo '<a href="commision.php?procedure=comms_editme&cuid='.$Arrayline['uid'].'" title="This will Edit me"><i class="fa fa-pencil-square fa-2x" aria-hidden="true"></i></a>';
		 echo '</td>';
		echo '</tr>';
  ...
  echo '</table>';
    ...
    //echo '<a href="commision.php?procedure=addnewcommlevel"><i class="fa fa-fw fa-plus fa-2x"></i>Add Commission Level</a>';
    echo '<button class="btn success"><a href="commision.php?procedure=addnewcommlevel"><i class="fa fa-handshake-o fa-lg" aria-hidden="true"></i> Add Commission Level</a></button>';
	...
	if ($cpercentage > 50){
	  ...
	  $myerror .= "<br><br><br><BIG><BIG><BIG>Commision can NOT be bigger then 50% of the tour price</BIG></BIG></BIG>";
        ...
        if ($cpercentage < 1){
            ...
            $myerror .= "<br><br><br><BIG><BIG><BIG>Commision can NOT be Less than 1% of the tour price</BIG></BIG></BIG>";
...
echo '
  <form action="commision.php?procedure=addnewcommlevel" method="post">
...
if ($myerror=="" && $wasiposted <> ""){
   ...
   //echo "Fourth<HR>";
	...
	echo '<meta http-equiv="refresh" content="0;url=commision.php?procedure=commission&showSnack=Record added successfully..." />';
...
if ($procedure=="comms_editme"){
   ...
   ////echo "<H1>".$tuid."</H1>";
	...
	if ($fromDB==""){
		...
		$query = "SELECT * FROM commission WHERE uid='".$cuid."'";
		$result = mysqli_query($dbConn, $query);
		while($Arrayline = mysqli_fetch_assoc($result)) {
...
if ($wasiposted <> ""){
	...
	if ($cpercentage > 50){
	  ...
	  $myerror .= "<br><BIG><BIG>Commision can NOT be bigger then 50% of the tour price</BIG></BIG></BIG>";
	...
	if ($cpercentage == 0){
	  ...
	  $myerror .= "<br><BIG><BIG>Commision must be more than Zero!!!</BIG></BIG></BIG>";
...
echo '
  <form action="commision.php?procedure=comms_editme&cuid='.$cuid.'" method="post">
  ...
  <input type="submit" value="Submit">
   ...
   if ($myerror=="" && $fromDB <> ""){
	...
	echo '<meta http-equiv="refresh" content="0;url=commision.php?procedure=commission&showSnack='.$Commname.'- Record updated successfully..." />';
